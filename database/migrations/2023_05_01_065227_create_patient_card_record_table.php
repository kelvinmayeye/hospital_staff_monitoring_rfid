...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('patient_card_record', function (Blueprint $table) {
            ...
            $table->foreign('scanner_id')->references('id')->on('card_scanners')->onDelete('restrict');
            $table->unsignedBigInteger('patient_card_id');
            $table->foreign('patient_card_id')->references('id')->on('patient_card')->onDelete('restrict');
            $table->string('status');
            $table->timestamps();
        });
    }
